...
def extreme_moves(m1: pd.DataFrame, n: int = 12) -> pd.DataFrame:
    """Largest |close-to-close| M1 moves (%) with timestamps.

    Surfaces bad prints, clipped ticks, and contract-rollover jumps for the per-instrument review
    (e.g. the WTIUSD April-2020 volatility and any garbage spike). Reported, never patched.
    """
    c = m1["close"]
    prev = c.shift(1)
    pct = ((c - prev) / prev).abs() * 100.0
    idx = pct.sort_values(ascending=False).head(n).index
    return pd.DataFrame({
        "at": idx,
        "prev_close": prev.loc[idx].to_numpy(),
        "close": c.loc[idx].to_numpy(),
        "pct_move": pct.loc[idx].to_numpy().round(3),
    }).reset_index(drop=True)
...
def _plot_bars_per_year(per_year: pd.DataFrame, out_png: Path, symbol: str = "XAUUSD") -> None:
    import matplotlib
    matplotlib.use("Agg")
    ...
    ax.set_title(f"{symbol} M1 bars per year (red = thin vs median)")
...
def build_report(
    ...
    # XAUUSD keeps the original unsuffixed artifacts; replication instruments are suffixed.
    suffix = "" if cfg.symbol == "XAUUSD" else f"_{cfg.symbol}"
    out_dir = cfg.output_dir / "quality"
    qdir = out_dir if cfg.symbol == "XAUUSD" else out_dir / cfg.symbol
    ...
    per_year.to_csv(qdir / "per_year.csv")
    rs.to_csv(qdir / "resample_counts.csv", index=False)
    big.to_csv(qdir / "largest_gaps.csv", index=False)
    em.to_csv(qdir / "extreme_moves.csv", index=False)

    png = repo / "assets" / f"data_quality_bars_per_year{suffix}.png"
    _plot_bars_per_year(per_year, png, cfg.symbol)

    md = repo / "docs" / f"DATA_QUALITY{suffix}.md"
    lines: List[str] = []
    ...
    lines.append("## Largest M1 moves (bad-print / rollover scan)\n")
    lines.append("Largest |close-to-close| M1 moves, reviewed for clipped/garbage prints and "
                 "contract-rollover jumps (reported, not patched).\n")
    lines.append("| at (UTC) | prev_close | close | % move |")
    lines.append("|---|---:|---:|---:|")
    for _, r in em.iterrows():
        lines.append(f"| `{r['at']}` | {r['prev_close']:.5g} | {r['close']:.5g} | {r['pct_move']:.3f}% |")
    lines.append("")
    ...
    return {"markdown": md, "plot": png, "per_year_csv": qdir / "per_year.csv",
            "resample_csv": qdir / "resample_counts.csv",
            "gaps_csv": qdir / "largest_gaps.csv", "extreme_csv": qdir / "extreme_moves.csv"}
